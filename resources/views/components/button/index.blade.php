...
<button
    ...
    flex items-center justify-center space-x-2 disabled:bg-fuchsia-400/75 disabled:cursor-wait"
    ...
    @endif
>
    <x-loading {{ $attributes->whereStartsWith(['wire:target']) }} wire:loading class="text- h-5 w-5" />
    <span>{{ $text ?? $slot ?? '' }}</span>
</button>